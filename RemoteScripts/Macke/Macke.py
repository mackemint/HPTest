...
class Macke(ControlSurface):
    ...
    def _setup_transport_control(self):
        ...
        #Setting up transport controls here
        #ButtonElement(is_momentary, msg_type, channel, identifier)
        
        transport.set_play_button(ButtonElement(True, MIDI_NOTE_TYPE, midi_channel, play_button)) 
        transport.set_stop_button(ButtonElement(True, MIDI_NOTE_TYPE, midi_channel, stop_button))
        transport.set_record_button(ButtonElement(True, MIDI_NOTE_TYPE, midi_channel, rec_button))
    ...
    def pushButton(self, channel, value):
        if (value != -1): 
            return ButtonElement(MIDI_NOTE_TYPE, channel, value) # assign data to slider object
        else:
            return None    
...
#        // Track properties
		...
		strip.set_send_controls((EncoderElement(MIDI_CC_TYPE,midi_channel,send_a[index],Live.MidiMap.MapMode.absolute),EncoderElement(MIDI_CC_TYPE,midi_channel,send_b[index],Live.MidiMap.MapMode.absolute)))  
